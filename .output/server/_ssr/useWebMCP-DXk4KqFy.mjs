...
import { a as assertHandoffPolicyAllows } from "./Card-BvuBlAzF.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/useWebMCP-DXk4KqFy.js
...
function getWebMCPPresentation(state) {
	switch (state.status) {
		...
		case "unavailable": return {
			...
			detail: "This browser does not expose document.modelContext. Use Chrome 149+ with chrome://flags/#enable-webmcp-testing enabled, then relaunch. ShowOnce does not simulate tools."
		};
	}
}
...
var SHOWONCE_TOOLS = [
	...
	},
	{
		...
		description: "Sets the recipient renewal frequency to monthly or annual. The connected Northstar Benefits page updates immediately.",
		inputSchema: objectSchema({ period: {
			type: "string",
			enum: ["monthly", "annual"]
		} }, ["period"]),
		annotations: { readOnlyHint: false },
	...
	},
	{
		...
		description: "Turns paperless communication on or off. The connected Northstar Benefits page updates immediately.",
	...
	try {
		for (const descriptor of SHOWONCE_TOOLS.filter(({ scopes }) => supportsScope(scopes, context.scope))) {
			const tool = {
				...
				execute: (input, invocationOptions) => invoke(descriptor, context, input, invocationOptions?.signal ?? neverAbortedSignal)
			};
